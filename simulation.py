...
from objects import StationaryObject
...
class Simulation:
    def __init__(self, screen_rect: Rect):
        ...
        self.stationary_objects: list[
            StationaryObject
        ] = []  # TODO: add other objects to affect the boids
    ...
    def get_neighbors_for_point(self, point: Vector2, radius: float) -> Iterator[Boid]:
        return (
            self.boids[i]
            for i in self.boids_tree.query_ball_point(np.array(point), radius, p=2.0)
        )
